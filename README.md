...
Bind방식으로 하는 대신에 GameState에 있는 시간 관련 변수를 Replicate설정하고, 위젯의 NativeTick에서 매 Tick마다 함수를 통해 값을 받아와 변환하여 보여주도록 하였음.

Tick대신 반복 가능한 Timer객체를 사용한다면 추가적인 개선이 가능할것
...
즉 매번 채팅 입력시 조건에 부합하는 (게임이 진행중이고, 3자리이고, 단어가 모두 숫자이고, 해당 플레이어의 턴이라면) 채팅이라면 답을 낸것으로 간주하고 

그렇지 않다면 일반 채팅으로 간주하여 로직을 진행함.
...
**기타 구현 사항**
   ...
   **플레이어 입장/퇴장시의 게임 초기화 및 대기 기능추가.**
   
   게임은 플레이어가 2명 이상일 때 시작되며, 2인 미만이라면 다른 플레이어가 로그인 할 때 까지 대기함.
   
   플레이 중 입장 또는 퇴장으로 플레이어 수가 변동될 경우 알림과 함께 게임을 재시작 시도하며,
   
   퇴장시 인원이 2명 미만이면 대기상태로 돌아감.
   ...
   **플레이어 닉네임 설정**
   ...
   관리중인 플레이어 TArray를 해당 함수에서 관리하도록 하고 이후 플레이어 접속 관련 로직도 해당 함수에서 실행하도록 하였음.
